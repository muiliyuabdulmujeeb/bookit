...
@auth_router.post("/me", response_model= UpdateAccountResponse)
...
async def sign_in_route(db: db_dependency, user_details: SignIn):
    return await sign_in(db=db, user_details= user_details)

@auth_router.post("/logout")
async def sign_out_route(db: db_dependency, token: token_dependency):
    result = await sign_out(db= db, token= token)
    await db.commit()
    return result
